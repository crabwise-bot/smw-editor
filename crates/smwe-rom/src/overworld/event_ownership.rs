...
impl EventOwnership {
    ...
    pub fn event_for(&self, translevel: usize) -> anyhow::Result<Option<u8>> {
        let byte =
            self.table.get(translevel).copied().ok_or_else(|| {
                anyhow::anyhow!("translevel {translevel:#04X} out of range (0..{EVENT_OWNERSHIP_COUNT})")
            })?;
...
pub(crate) fn vanilla_table() -> Vec<u8> {
    vec![
        0xFF, 0x1F, 0x20, 0xFF, 0x0B, 0x0D, 0x0E, 0x0F, 0x28, 0x09, 0x10, 0x21, 0x22, 0x23, 0x24, 0x25, 0x27, 0x60,
        0xFF, 0x12, 0x02, 0x07, 0xFF, 0xFF, 0x4E, 0xFF, 0x4D, 0x4A, 0x4C, 0x4B, 0x36, 0x35, 0x61, 0x63, 0x62, 0x48,
        0x46, 0x06, 0x05, 0x04, 0x00, 0x01, 0x03, 0x19, 0xFF, 0x1D, 0x1A, 0x14, 0x44, 0x45, 0x42, 0x3E, 0x40, 0x41,
        0x43, 0x3D, 0x3B, 0x39, 0x38, 0x4F, 0x17, 0x1B, 0x15, 0x29, 0x1C, 0x30, 0x2A, 0x32, 0x2C, 0x37, 0x34, 0x2E,
        0x6D, 0x6C, 0x6B, 0x6A, 0x69, 0x64, 0x65, 0x66, 0x67, 0x68, 0x56, 0x53, 0x54, 0x5F, 0x57, 0x59, 0x51, 0x5A,
        0x5D, 0x50, 0x5C,
    ]
}
