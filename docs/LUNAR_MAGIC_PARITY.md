...
| Player (Mario/Luigi) palette editing | ✅ (beyond Lunar Magic) | `crates/smwe-rom/src/player_palette.rs` parses the 4×10-color `PlayerColors` table at SNES $00B2C8 (Mario/Luigi × normal/fire, chosen at runtime by `DATA_00E2A2` and DMA'd to CGRAM sprite row 8 by `MarioGFXDMA`); the level editor's Palette Editor has a new "Player Colors" section with per-palette tabs, a 10-swatch grid, and a color picker, saved to ROM through the existing Ctrl+S path. Verified against real ROM: parsed bytes match SMWDisX's `col/misc/*_{normal,fire}.pal` via its `%incpal` conversion byte-for-byte. Lunar Magic has no editor for these palettes |
